  ...
  implements OnGatewayConnection, OnGatewayDisconnect, OnGatewayInit
{
  ...
  async handleMessage(
    ...
    try {
      ...
      const isParticipant = dbRoom.participants.some(
        (p) => p.userId === user.id
      );
      if (!isParticipant) {
        console.error(
          ...
          if (dbMessage.driverSender) {
            senderDetails = {
              ...
              contact_phone: formatContact(
                dbMessage.driverSender.contact_phone
              ),
            };
          }
          break;
  ...
  async handleSessionEnded(data: {
    ...
    console.log(
      `Support session ended: ${data.sessionId}, duration: ${Math.round(
        ...
        for (const participantId of data.initialParticipants) {
          ...
          if (participantSocket) {
            ...
            participantSocket.emit("adminGroupCreated", {
              ...
              createdByName: `${userData.first_name || ""} ${
                userData.last_name || ""
              }`.trim(),
            });
          }
      ...
      for (const invitedUserId of data.invitedUserIds) {
        ...
        if (invitedSocket) {
          invitedSocket.emit("groupInvitationReceived", {
            ...
            inviterName: `${userData.first_name || ""} ${
              userData.last_name || ""
            }`.trim(),
            message: data.message,
          });
    ...
    try {
      ...
      if (otherUserSocket) {
        ...
        otherUserSocket.emit("directAdminChatStarted", {
          ...
          withUser: userData.id,
          withUserName: `${userData.first_name || ""} ${
            userData.last_name || ""
          }`.trim(),
          category: data.category,
          priority: data.priority,
    ...
    try {
      ...
      this.server
        ...
        .emit("groupSettingsUpdated", {
          ...
          updatedByName: `${userData.first_name || ""} ${
            userData.last_name || ""
          }`.trim(),
          changes: data,
          timestamp: new Date().toISOString(),
  ...
  async handleManageGroupParticipant(
    ...
    try {
      ...
      this.server.to(`admin_group_${data.groupId}`).emit("participantManaged", {
        ...
        managerName: `${userData.first_name || ""} ${
          userData.last_name || ""
        }`.trim(),
        reason: data.reason,
        timestamp: new Date().toISOString(),
